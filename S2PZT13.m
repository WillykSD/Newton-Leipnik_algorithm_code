...
beta1 = -0.16  ;  beta2 = -1.6;  delta = 0.001;   eta = -64.0; f0=.02;  omega=7;   
...
a0 = 0.0:0.00001:0.02; %a0 = 0.13:0.00001:0.17; %a0 = 0.0:0.00001:0.04; %a0 = 0.040:0.00001:0.18; 
...
for i=1:n
    for j=1:m-1
        ...
        if (e1*e2<=0)
            ...
            P1maxP(jj,i)=b^2*omega^2; P2maxP(jj,i)=c^2; 
        end
        %
        %
        ...
        if (e1*e2<=0)
            ...
            b=abs(beta0+(1/4)*beta2*y^2)*omega*y/sqrt((r1(x)*varsigma2(x)-1)^2+(mue*omega+r1(x)*varsigma1(x))^2);
            c=abs(eta)*omega^2*y/sqrt((r2*varrho2-omega^2)^2+(muP*omega+r2*varrho1)^2);
            P1maxM(jj,i)=b^2*omega^2; P2maxM(jj,i)=c^2; 
        end 
    end
...
figure(1);hold on
mesh(XX,YY,P1max); xlabel('\epsilon'); ylabel('\theta'); zlabel('P_{1max}');

figure(2);hold on
mesh(XX,YY,P2max); xlabel('\epsilon'); ylabel('\theta'); zlabel('P_{2max}');
